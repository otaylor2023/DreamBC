...
CTRL_WORLD_ROOT = Path(__file__).resolve().parents[2]
...
class Dataset_xhand(Dataset):
    ...
    def fetch(self,index):
        ...
        # load state, action
        self.old_path = str(getattr(self.args, 'raw_droid_path', CTRL_WORLD_ROOT / 'dataset_example/droid_source'))
...
class Args:
    def __init__(self):
        self.data_json_path = str(CTRL_WORLD_ROOT / 'dataset_meta_info/droid')
        self.data_root_path = str(CTRL_WORLD_ROOT / 'dataset_example')
        self.raw_droid_path = str(CTRL_WORLD_ROOT / 'dataset_example/droid_source')
