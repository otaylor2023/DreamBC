...
def resolve_repo_path(path):
    ...
    if path.startswith("Ctrl-World/"):
        return str(CTRL_WORLD_ROOT.parent / path_obj)
    return str(CTRL_WORLD_ROOT / path_obj)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--droid_hf_path', type=str, default=str(CTRL_WORLD_ROOT / 'dataset_example/droid_source'))
    parser.add_argument('--droid_output_path', type=str, default=str(CTRL_WORLD_ROOT / 'dataset_example/droid_subset'))
    parser.add_argument('--svd_path', type=str, default='stabilityai/stable-video-diffusion-img2vid')
    # debug
    parser.add_argument('--debug', action='store_true')
    args = parser.parse_args()
    args.droid_hf_path = resolve_repo_path(args.droid_hf_path)
    args.droid_output_path = resolve_repo_path(args.droid_output_path)
