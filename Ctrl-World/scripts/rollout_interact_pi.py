...
import sys, os
CTRL_WORLD_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OPENPI_ROOT = os.path.join(CTRL_WORLD_ROOT, "openpi")
sys.path[:0] = [
    CTRL_WORLD_ROOT,
...
class agent():
    def __init__(self,args):
        ...
        # load pi policy
        pi_train_config = getattr(args, "pi_train_config", None) or "default"
        if pi_train_config in ("dreambc_lora", "dreambc_full"):
            # Custom TrainConfig matching what scripts/train_dreambc.py produced.
            # We use this when --pi_ckpt points at a fine-tuned LoRA or full
            # DreamBC checkpoint directory so the model variant matches the
            # weights on disk. Norm stats are still loaded from
            # <pi_ckpt>/assets/droid (handled by create_trained_policy).
            import sys as _sys, os as _os
            _DREAMBC_SCRIPTS = _os.path.join(
                _os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))),
                "scripts",
            )
            if _DREAMBC_SCRIPTS not in _sys.path:
                _sys.path.insert(0, _DREAMBC_SCRIPTS)
            from dreambc_data_config import make_dreambc_data_config  # type: ignore
            import openpi.models.pi0_config as _pi0_config
            import openpi.training.config as _opi_config
            import openpi.training.weight_loaders as _weight_loaders

            is_lora = pi_train_config == "dreambc_lora"
            ...
            config = _opi_config.TrainConfig(
                name="pi05_dreambc",
                exp_name="eval",  # not used by create_trained_policy
                model=model,
...
if __name__ == "__main__":
    from config import resolve_paths, wm_args
    ...
    default_pi_ckpt = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        "openpi/checkpoint/pi05_droid",
    )
    parser.add_argument('--pi_ckpt', type=str, default=default_pi_ckpt)
    parser.add_argument('--pi_train_config', type=str, default='default',
                        choices=('default', 'dreambc_lora', 'dreambc_full'),
                        help='default: use openpi pi05_droid/pi0_droid/pi0fast_droid config '
                             '(matches the released pi05 checkpoint). dreambc_lora: load a '
                             'LoRA fine-tune produced by scripts/train_dreambc.py --variant lora. '
                             'dreambc_full: load a full fine-tune produced by --variant full.')
    ...
    sweep_config = {
        ...
        "pi_train_config": str(getattr(args, "pi_train_config", "default")),
