"""Inspect and save embodiment alignment for the current robot + wrist camera setup."""
...
from dreambc_isaac.cameras import apply_camera_auto_alignments, make_cameras
from dreambc_isaac.debug import collect_camera_debug, collect_prim_world_positions
...
def main(cfg: DictConfig) -> None:
    ...
    output_root = resolve_project_path(project_root, "rollouts/embodiment_alignment")
    ...
    for _ in range(max(3, int(cfg.sim.warmup_steps))):
        if arm_joint_names:
            set_named_joint_position_targets(robot, arm_joint_names, configured_arm_home)
        if gripper_joint_names:
            set_named_joint_position_targets(robot, gripper_joint_names, configured_gripper_home)
    ...
    alignment_summary = apply_camera_auto_alignments(world.stage, cameras, cfg.cameras, cfg.debug.grasp_debug)
    camera_debug = collect_camera_debug(cameras, stage=world.stage, camera_cfg=cfg.cameras)
    ...
    summary = {
        ...
        "camera_auto_alignment": alignment_summary,
        "camera_world_poses": camera_debug,
        "prim_world_positions": prim_debug,
        "policy_gripper_calibration": {
            "command_open_target": float(cfg.policy.pi05_droid_remote.gripper_open_target),
            "command_closed_target": float(cfg.policy.pi05_droid_remote.gripper_closed_target),
            "observation_open_position": float(cfg.policy.pi05_droid_remote.gripper_observation_open_position),
            "observation_closed_position": float(cfg.policy.pi05_droid_remote.gripper_observation_closed_position),
        },
    }
    summary_path = output_dir / "embodiment_alignment.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"Saved embodiment alignment: {summary_path}")

    if not bool(cfg.sim.headless) and bool(cfg.sim.keep_open_after_rollout):
        print("Embodiment alignment complete. Isaac Sim UI will stay open; close the window to exit.")
