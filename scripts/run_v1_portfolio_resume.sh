#!/usr/bin/env bash
# Resume the v1 portfolio from the first failed/incomplete full-FT run.
#
# Usage:
#   ./scripts/run_v1_portfolio_resume.sh rollouts/C16_g5_block
set -uo pipefail
...
CHAIN_TAG="cube_c16_v1_portfolio_resume_${TIMESTAMP}"
MASTER_LOG="$REPO_ROOT/logs/${CHAIN_TAG}.log"
MARKER_DIR="$REPO_ROOT/training_runs/chain_markers"

mkdir -p "$REPO_ROOT/logs" "$MARKER_DIR"

if [[ "${DREAMBC_V1_PORTFOLIO_RESUME_DETACHED:-}" != "1" ]]; then
  export DREAMBC_V1_PORTFOLIO_RESUME_DETACHED=1
  echo "[portfolio-resume] starting detached -> $MASTER_LOG"
  setsid nohup "$0" "$BUNDLES_ROOT" > "$MASTER_LOG" 2>&1 < /dev/null &
  disown
  echo "[portfolio-resume] pid=$! master_log=$MASTER_LOG"
  exit 0
fi
...
date -Is > "$START_MARK"
echo "[portfolio-resume $(date -Is)] START resume chain=$CHAIN_TAG"
...
run_one() {
  ...
  echo "[portfolio-resume $(date -Is)] START $exp_name (variant=$variant max_steps=$max_steps aug=$enable_aug)"
  ...
  if [[ $rc -ne 0 ]]; then
    date -Is > "$fail_mark"
    echo "[portfolio-resume $(date -Is)] FAILED $exp_name rc=$rc (see $run_log)" | tee -a "$FAIL_MARK"
    return "$rc"
  fi

  date -Is > "$done_mark"
  echo "[portfolio-resume $(date -Is)] DONE $exp_name"
  return 0
}
...
date -Is > "$DONE_MARK"
echo "[portfolio-resume $(date -Is)] RESUME COMPLETE"

# Regenerate portfolio doc (same logic as main orchestrator).
...
for exp_dir in sorted(ckpt_root.glob("cube_c16_*")):
...
out_path.write_text("\n".join(lines) + "\n")
print(f"[portfolio-resume] wrote {out_path}")
PY

echo "[portfolio-resume $(date -Is)] portfolio doc -> $PORTFOLIO_DOC"
